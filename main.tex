...
\begin{proof}
    Niech $\zeta$ będzie pierwotnym pierwiastkiem $n$-tego stopnia nad $\mathbb{Z}_q$. Dowód przebiegał będzie w dwóch krokach: 
    \begin{enumerate}[label=\arabic*.,leftmargin=.4in]
        \item pokażemy, że dla dowolnego $k > 1$ $\zeta^{q^k}=\zeta$ wtw, gdy $\zeta \in \mathbb{Z}_{q^k}$ oraz, że 
        \item jeśli $\zeta \in \mathbb{Z}_{q^d}$ jest pierwiastkiem wielomianu $f(X) \in \mathbb{Z}_q$, to istnieje $h(X) \in \mathbb{Z}_q$ takie, że $h(X) \, | \, f(X)$ oraz $deg(h(X)) = d$.
    \end{enumerate}
	\begin{enumerate}[label=Ad.\arabic*.,leftmargin=.4in]
