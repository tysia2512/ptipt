...
\documentclass[polish,declaration,shortabstract]{iithesis}
...
\begin{proof}
	...
	\begin{enumerate}[label=\arabic*.,leftmargin=.4in]
		\item pokażemy, że dla dowolnego $k > 1$ $\zeta^{q^k}=\zeta$ wtw, gdy $\zeta \in \mathbb{F}_{q^k}$ oraz, że 
		\item jeśli $\zeta \in \mathbb{F}_{q^d}$ jest pierwiastkiem wielomianu $f(X) \in \mathbb{Z}_q[X]$, to istnieje $h(X) \in \mathbb{Z}_q[X]$ takie, że $h(X) \, | \, f(X)$ oraz $deg(h) = d$.
	\end{enumerate}
	\begin{enumerate}[label=Ad.\arabic*.,leftmargin=.4in]
		...
		\item Niech $m(X) \in \mathbb{Z}_q$ będzie minimalnym wielomianem dla $\zeta$. Wiemy, że taki istnieje i jest niezerowy, ponieważ istnieje $f(X) = X^{n} - 1 \in \mathbb{Z}_q[X]$ i $f(\zeta) = 0$. Niech $d = deg(m)$. Oznaczmy $\mathbb{Z}_q / \langle m(X) \rangle$ przez $\mathbb{F}_{p^d}$. Z własności wielomianu minimalnego mamy $m(X) \, | \, Q_n(X)$ oraz $m(X)$ jest nierozkładalny w $\mathbb{Z}_q$.
