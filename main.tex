...
\englishabstract{The purpose of this thesis is presentation of AKS algorithm in an understable way for a reader with basic knowledge of abstract algebra. It is an extension of the article \textit{PRIMES is in P} by M. Agrawal, N. Kayal and N. Saxena. It suplements the article with proofs of helper lemmas and theorems of algebra.

The beginning part serves as an introduction of terms of abstract algebra necessary to prove corectness of AKS algorithm, such as rings, fields, group of roots of unity and cyclotomic polynomials. It also contains proofs of theorems used in further chapters. This part is an introduction to following proof of correctness of AKS algorithm, which is main focus of this thesis. Presented proof suplements \textit{PRIMES is in P} with proofs of non-trival lemmas. Moreover all used theorems of algebra can be found in the introductory chapter. The thesis also contains an estimation for algorithm's time complexity. However, presented estimation is not the most accurate estimation known, as its main purpose is to proove polynomial complexity of algorithm.

Additionaly, a sample implementation of AKS algorithm in C++ language is presented.}
...
\begin{remark*}
    Jako że definiujemy nową operację, poza udowadnianymi do tej pory własnościami, musimy uargumentować najpierw zasadność takiej definicji działania. Należy pokazać, że, co do tej pory zakładaliśmy, wyniki działania na parach argumentów, których odpowiadające argumenty są sobie równe, także są równe.
\end{remark*}
...
Zauważmy, że ustalone powyżej liczby $n, \, r$ spełniają założenia, jakie spełniają odpowiednio zmienne $n$ i $r$ w 7. kroku algorytmu. Jeśli dodatkowo założymy, że $n$ jest złożona, to istnieje $p$ spełniające wszystkie założenia. Dowód będzie przebiegał przed oszacowanie mocy zbioru $\mathcal{G}$ z góry i z dołu doprowadzając do sprzeczności, skąd będziemy mogli wywnioskować, że niemożliwym jest, aby w algorytm zakończył się w kroku 7. przy złożonej $n$.
...
\begin{proof}
	...
	Rozważmy pierwiastki wielomianów $h(X)$ i $X^k - 1$ w $r$-tym ciele cyklotomicznym nad $\mathbb{Z}_p$. Istnieje w nim pierwiastek pierwotny $r$-tego stopnia $\zeta$, który jest pierwiastkiem $h(X)$. 
	% UWAGA TO MOŻE BYĆ NIEPRAWDA
	Mamy więc $h(\zeta) = 0 \; (mod \, p)$ w $\mathbb{Z}_p^{(r)}$, a ponieważ $h(X) \, | X^k - 1$, to także $\zeta^k - 1 = 0 \; (mod \, p)$ w $\mathbb{Z}_p^{(r)}$. Wynika stąd, że $\zeta$ jest też pierwiastkiem z jedności $k$-tego stopnia w $\mathbb{Z}_p$, a ponieważ $k < r$, mamy sprzeczność z założeniem o pierwotności $\zeta$.
...
\begin{lemma}\label{liczba_wielo}
	Dla $k + 1$ wielomianów pierwszego stopnia o różnych pierwiastkach $f_1(X), \, \dots,$ $\,f_{k + 1}(X)$ istnieje co najmniej ${k + d \choose k + 1}$ różnych wielomianów $f(X)$ stopnia mniejszego niż $d$, które można przedstawić jako $f(X) = \prod_{i=1}^{k+1}(f_i(X))^{e_i}$, gdzie $e_i \in \mathbb{N}$.
\end{lemma}
...
Udowodniwszy pomocnicze lematy, możemy przejść do znalezienia ograniczenia dolnego mocy zbioru $\mathcal{G}$.
...
\begin{proof}
	...
	\begin{enumerate}[label=Ad.\arabic*.,leftmargin=.4in]
		...
		\item Z założeń o $\ell$ i $o_r(n)$ mamy $\ell = \lfloor \sqrt{\phi(r)} \, log\, n \rfloor < \sqrt{r} \, log \, n $ oraz $o_r(n) > log^2 \, n$. Ponieważ $r > o_r(n)$, otrzymujemy \[\ell < \sqrt{r} \, log \, n < \sqrt{r \cdot o_r(n)} < r < p.\] W połączeniu z $deg(h) > 1$ mamy wniosek że dla dowolnych $0 \leq i < j \leq \ell$ zchodzi $X + i \neq X + j$ w $F$ oraz $X + i$ i $X + j$ są niezerowe. 
...
\begin{proof}
	Weźmy zbiór $A$ liczb względnie pierwszych z $r$, mniejszych od $r$. Z definicji funkcji Eulera mamy $| \, A \, | = \phi(r)$. Niech \[B = \{b \, | \, b = p \cdot a \, (mod \, r), \, b < r, \, a \in A\},\]. Zauważmy, że dla wszystkich $b \in B$ zachodzi $NWD(b, r) = 1$, więc $B \subseteq A.$ Pokażemy, że $A = B$. Załóżmy nie wprost $p \cdot a_i = p \cdot a_j \, (mod \, r)$, gdzie $a_i, a_j \in A$. Z Lematu \ref{odwrotny} wiemy, że istnieje $p^{-1} \in \mathbb{Z}_r$. Więc mnożąc stronami przez $p^{-1}$ otrzymujemy sprzeczność. \newline
	Niech $A = \{a_1, \ldots, a_{\phi(r)}\}$. Mamy $A = B$, możemy więc wywnioskować równanie 
	\begin{align*}
...
\begin{lemma}\label{el_odwr}
	$\langle G, \cdot \rangle$ jest podgrupą $\langle \mathbb{Z}_r \setminus \{0\}, \cdot \rangle$ oraz $|\,G\,| > log^2n$.
\end{lemma}
	
\begin{proof}
	Dowód przebiegać będzie w dwóch krokach.
	\begin{enumerate}[label=\arabic*.,leftmargin=.4in]
		\item Udowodnimy, że $\langle G, \cdot \rangle$ spełnia definicję grupy i zawiera się w $\mathbb{Z}_r \setminus \{0\}$.
		      \begin{enumerate}[label=1.\arabic*.]
		      	\item Oczywistym jest, że $G \subseteq \mathbb{Z}_r$. Wiemy, że $NWD(n, \, r) = 1$ oraz $p \, | \, n$, więc $NWD(p, \, r) = 1$. Wynika stąd, że nie istnieje w $I$ element podzielny przez $r$, więc $0 \notin G$. Mamy więc $G \subseteq \mathbb{Z}_r \setminus \{0\}$. 
		      ...
		      \end{enumerate}
		\item Pokażemy, że $|\,G\,| > log^2n$. Załóżmy nie wprost, że $|\,G\,| \leq log^2n$. Spójrzmy na ciąg $1,\, n, \, \dots\,, n^{|G|}$ modulo $r$. Jest to ciąg $|G| + 1$ liczb należących do $G$. Wynika stąd, że istnieją $k, l \in \mathbb{N}, 0 \leq k < l \leq |\,G\,|$ takie, że $n^k = n^l \, (mod\,r)$. Mamy więc $n^{l-k} = 1 \, (mod\,r)$. Wówczas $l - k \leq |\,G\,| \leq log^2n$, co jest sprzeczne z założeniem, że $o_r(n) > log^2n$. 
	\end{enumerate}
\end{proof}
		...
		\item Załóżmy nie wprost, że algorytm zakończył wykonanie w kroku 7., zwracając \textit{PIERWSZA} i $n$ jest złożona. Ponieważ algorytm nie zakończył się w kroku 1., wiemy, że $n$ nie jest potęgą żadnej liczby naturalnej, w szczególności nie istnieją takie $p < n, k \in \mathbb{N}$, gdzie $p$ jest pierwsze, że $n = p^k$. W kroku 2. zostaje wybrane najmniejsze takie $r$, że $o_r(n) > log^2n$. Ponadto z niespełnionego warunku w kroku 3. wiemy, że dla $1 \leq a \leq r$ zachodzi $NWD(a, \, n) = 1$, w szczególności $NWD(r, \, n) = 1$. Z kroków 4. i 5. mamy $n > r$ oraz ${\forall_{1 \leq a \leq \lfloor \sqrt{\phi(r)} \, log \, n \rfloor} \, (X + a)^n = X^n + a \: (mod \, X^r - 1, \, n).}$ Z założenia, że $n$ jest liczbą złożoną wiemy, że istnieje $p$, będące pierwszym dzielnikiem $n$. Mamy więc $n, r, p \in \mathbb{N}$, spełniające założenia w Uwadze \ref{def}. Weźmy zdefiniowany w niej zbiór $\mathcal{G}$. Na mocy Lematu \ref{lower} mamy nierówność $| \, \mathcal{G} \, | \geq {t + \ell \choose t - 1}$ oraz z Lematu \ref{el_odwr} oraz Uwagi \ref{def} zachodzi ${t > log^2n,}$ ${\ell = \lfloor \sqrt{\phi(r)}log \, n \rfloor.}$ Możemy więc wywnioskować nierówność
		      \begin{align*}
		      	...
		      	& = {\lfloor \sqrt{t}\, log \, n \rfloor + 1 + \ell \choose \lfloor \sqrt{t}\, log \, n \rfloor} \\
		      	  & \geq {2\lfloor \sqrt{t}\, log \, n \rfloor + 1 \choose \lfloor \sqrt{t}\, log \, n \rfloor} &   & \text{Z Lematu \ref{phi_r} otrzymujemy $\ell \geq \lfloor \sqrt{t} \, log \, n \rfloor$.} \\
		      	  & > 2^{\lfloor \sqrt{t}\, log \, n \rfloor + 1}                                               &   & \text{Z Lematu \ref{newton1}.}                                                                                                                 \\
		      	&\geq 2^{\sqrt{t}\,log \,n} \\
		      	&= n^{\sqrt{t}}.
		      \end{align*}
		      Mamy więc $| \, \mathcal{G} \, | > n ^{\sqrt{t}}$ oraz, ponieważ $n$ nie jest potęgą liczby pierwszej, z Lematu \ref{upper} mamy $| \, \mathcal{G} \, | \leq n^{\sqrt{t}}.$ Otrzymaliśmy sprzeczność, więc $n$ nie jest liczbą złożoną.
	\end{enumerate}
\end{proof}
...
\begin{proof}
	...
	Istnieje więc pewne $q$ takie, że $q \, | \, r$ i $q \nmid P$. Skoro $\lfloor log B \rfloor \geq 1$, możemy wywnioskować, że $q \nmid n$. Wynika stąd, że $q \nmid NWD(r,\, n)$, więc $\frac{r}{NWD(r, \, n)} \nmid P$. Znaleźliśmy więc liczbę $\frac{r}{NWD(r, \, n)} \leq r$, która nie dzieli $P$. Z założenia, że $r$ jest najmniejszą liczbą, która nie dzieli $P$, mamy $NWD(r, \, n) = 1$.\newline
...
\begin{proof}
	...
	\begin{enumerate}[label=(krok \arabic*.),leftmargin=.4in]
		...
		\item W kroku 5. wykonany zostanie krok 6. $\lfloor \sqrt{\phi(n)} \, log \, n \rfloor$ razy. Mamy więc złożoność obliczeniową kroku 5. \[O(\sqrt{\phi(r)} \, log \, n \cdot log^7n) \subseteq O(\sqrt{r} \, log \, n \cdot log^7n) \subseteq O(log^{\frac{5}{2}}n \cdot log^8n) \subseteq O(log^{\frac{21}{2}}n).\]
	\end{enumerate}				
