...
\begin{proof}
	...
	(1) Niech $0 \leq a \leq \ell$. $p$ jest pierwsze, więc z lematu \ref{L1} otrzymujemy \[(X + a)^p = X^p + a \, (mod \, X^r - 1, \, p),\] więc $p$ jest introspektywne dla $(X + a)$. Z założenia mamy też  \[(X + a)^n = X^n + a \: (mod \, X^r - 1, \, n).\] Weźmy $f_1(X) = (X + a)^{\frac{n}{p}}, \, f_2(X) = X^{\frac{n}{p}} + a \in \mathbb{Z}_p[X]$. Zauważmy, że 
	\begin{align*}
...
\begin{proof}
	Z lematu \ref{cyklo_dzieli} oraz ponieważ $h(X) \, | Q_r(X)$, mamy $h(X) \, | \, X^r - 1$, więc $X^r = 1$ w $F$, czyli $X$ jest pierwiastkiem $r$-tego stopnia z jedności w $F$. Załóżmy nie wprost, że $X$ nie jest pierwotnym pierwiastkiem. Oznacza to, że istnieje $k < r$ takie, że $X^k = 1$ w $F$. Implikuje to, że $h(X) | X^k - 1$ w $\mathbb{Z}_p[X]$. Rozważmy $h(X)$ i $X^k - 1$ w $r$-tym ciele cyklotomicznym nad $\mathbb{Z}_p$. Istnieje w nim pierwiastek pierwotny $r$-tego stopnia $\zeta$, który jest pierwiastkiem $h(X)$. Ponieważ $h(X) | X^k - 1$ także w rozszerzeniu ciała $\mathbb{Z}_p$, to $\zeta^k - 1 = 0$ w $\mathbb{Z}_p^{(r)}$. Otrzymaliśmy sprzeczność z założeniem, że $\zeta$ jest pierwiastkiem pierwotnym, ponieważ $k < r$.
...
\begin{lemma}\label{liczba_wielo}
	Jeśli w grupie $G$ istnieje co najmniej $k + 1$ różnych wielomianów $f_1(X), \, \dots,$ $\,f_{k + 1}(X)$ pierwszego stopnia niepodzielnych przez siebie parami, to istnieje co najmniej ${k + d \choose k + 1}$ różnych wielomianów stopnia mniejszego niż $d$.
\end{lemma}
